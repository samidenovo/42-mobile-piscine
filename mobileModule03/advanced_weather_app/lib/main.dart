    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Icon(getWeatherIcon(current['weather_code']), size: 80, color: Colors.blue),
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (value, meta) {
                    final i = value.toInt();
                    if (i < 0 || i >= todayIndexes.length) return Text('');
                    return Text(i.toString(), style: TextStyle(fontSize: 10));
                  },
                ),
              ),
            ),
            lineBarsData: [LineChartBarData(
              spots: todayIndexes.asMap().entries.map((e) =>
                FlSpot(e.key.toDouble(), (hourly['temperature_2m'][e.value] as num).toDouble())
              ).toList(),
              isCurved: true,
              color: Colors.blue,
              barWidth: 3,
      ...
      children: [
          ...
          child: LineChart(LineChartData(
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (value, meta) {
                    final i = value.toInt();
                    if (i < 0 || i >= daily['time'].length) return Text('');
                    return Text(daily['time'][i].toString().substring(8, 10), style: TextStyle(fontSize: 10));
                  },
                ),
              ),
            ...
            lineBarsData: [
              LineChartBarData(
                spots: List.generate(daily['time'].length, (i) =>
                  FlSpot(i.toDouble(), (daily['temperature_2m_min'][i] as num).toDouble())),
                isCurved: true, color: Colors.blue, barWidth: 3,
                dotData: FlDotData(show: false),
              ),
              LineChartBarData(
                spots: List.generate(daily['time'].length, (i) =>
                  FlSpot(i.toDouble(), (daily['temperature_2m_max'][i] as num).toDouble())),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return ListTile(
                title: Text(['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][DateTime.parse(daily['time'][index]).weekday - 1]),
